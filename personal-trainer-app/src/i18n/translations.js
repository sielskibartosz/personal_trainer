const translations = {
  pl: {
    navbar: {
      about: "O mnie",
      exercises: "Ćwiczenia",
      pricing: "Cennik",
      contact: "Kontakt",
      title: "Bartosz Sielski - Treningi Personalne"
    },
    about: {
      title: "O mnie",
      hook: "Sprawność · Zdrowie · Sylwetka",
      highlight_1: "Trenuję z Tobą",
      highlight_2: "Uczę wzorców ruchowych i techniki",
      highlight_3: "Stawiam na zdrową, trwałą sprawność, a nie szybkie efekty",
      text: `Prowadzę treningi personalne ukierunkowane na poprawę ogólnej sprawności, kondycji i samopoczucia. Specjalizuję się w treningach siłowych, funkcjonalnych i motorycznych.

Podczas zajęć nie tylko ćwiczymy — uczę również, jak samodzielnie planować treningi, rozumieć wzorce ruchowe i bezpiecznie korzystać z siłowni. Dzięki temu zyskasz wiedzę i pewność, by trenować skutecznie także na własną rękę.

Jeśli chcesz poprawić zdrowie, kondycję i sylwetkę — zapraszam! Dla chętnych organizuję także wyjazdy rowerowe i górskie, a czasem zamiast siłowni proponuję squasha lub ściankę wspinaczkową.`
    },
    exercises: {
      title: "Ćwiczenia",
      stretching: "Stretching",
      strength: "Siła",
      core: "Stabilizacja",
      cardio: "Wytrzymałość"
    },
    back: "Wróć",
    categoryDescriptions: {
      stretching: `Stretching to nieodłączna część każdego mojego treningu i jest dla niezwykle ważny. 
Pomaga rozluźnić napięte mięśnie, redukuje stres mięśniowy, wspiera relaksację i poprawia zakres ruchu w stawach. 
Dzięki regularnemu rozciąganiu zwiększamy świadomość własnego ciała i kształtujemy prawidłową postawę. 
Szczególnie dbam o to u osób pracujących długo przy biurku, dla których jest to niezastąpiony element treningu, wspierający zarówno zdrowie, jak i komfort codziennego funkcjonowania.`,

      strength: `Regularne ćwiczenia siłowe wzmacniają mięśnie, kości i stawy, wspierają metabolizm oraz pomagają w utrzymaniu prawidłowej masy ciała. 
Silne i proporcjonalnie rozwinięte mięśnie zwiększają stabilność i pewność siebie podczas codziennych czynności.
Szczególna uwaga należy się mięśniom nóg, ponieważ ich wzmocnienie wspiera cały organizm, 
poprawia równowagę, stabilność oraz ułatwia ruchy dnia codziennego, takie jak chodzenie, bieganie czy wchodzenie po schodach.
Trening siłowy w moich zajęciach obejmuje całe ciało – nie faworyzuję żadnej partii mięśniowej, 
ponieważ dysproporcje mogą prowadzić do wad postawy i zwiększać ryzyko kontuzji. `,

      core: `Trening mięśni core, czyli głębokich mięśni brzucha i pleców, jest kluczowy dla stabilności całego ciała. 
Silny core, a w szczególności mocny brzuch, wspiera zdrowy kręgosłup, poprawia równowagę i koordynację, 
a także zwiększa efektywność w ćwiczeniach wielostawowych, takich jak przysiady, martwy ciąg czy pompki. 
Ćwiczenia core pomagają wykonywać codzienne ruchy sprawniej i bez ryzyka kontuzji. 
Są polecane także dzieciom i młodzieży, ponieważ wspierają rozwój prawidłowej postawy, 
wzmacniają mięśnie stabilizujące kręgosłup i poprawiają koordynację ruchową. 
Regularne wzmacnianie core od najmłodszych lat sprzyja zdrowemu rozwojowi fizycznemu i zapobiega problemom z plecami w przyszłości.
Na moich treningach korzystamy z gum oporowych, piłek fitness oraz innych akcesoriów, które świetnie wspomagają pracę nad mięśniami głębokimi.`,

      cardio: `Ćwiczenia takie jak bieganie, taniec, jazda na rowerze czy skakanka, zwiększają wydolność serca i płuc, 
wspomagają spalanie kalorii i poprawiają kondycję fizyczną. 
Wytrzymałość jest niezwykle ważna – pomaga spalać kalorie, poprawia krążenie i wzmacnia odporność. 
Jestem zwolennikiem cardio w formie, która sprawia przyjemność – taniec, squash, wyjścia w góry czy zwykłe spacery to według mnie najlepsze sposoby na aktywność, 
która daje radość i efekty. 
Dla chętnych organizuję wyjazdy w góry, rowery lub inne formy ruchu, które w przyjemny sposób zastępują bieganie na bieżni.`
    },
    pricing: {
      title: "Cennik",
      one: "Trening personalny",
      pack: "Pakiet 10 treningów",
      first: "Pierwszy trening",
      pair: "Trening w parze"
    },
    contact: {
      title: "Kontakt",
      phone: "Telefon",
      email: "Email"
    }
  },

  en: {
    navbar: {
      about: "About Me",
      exercises: "Exercises",
      pricing: "Pricing",
      contact: "Contact",
      title: "Bartosz Sielski - Personal Training"
    },
    about: {
      title: "About Me",
      hook: "Fitness · Health · Performance",
      highlight_1: "I train with you",
      highlight_2: "I teach movement patterns and proper technique",
      highlight_3: "I focus on healthy, long-term fitness, not quick fixes",
      text: `I provide personal training focused on improving overall fitness, strength, and well-being. I specialize in strength, functional, and athletic training.

During sessions, we do more than just exercise — I teach how to plan workouts, understand movement patterns, and train safely on your own. This gives you the knowledge and confidence to train effectively by yourself.

If you want to improve health, fitness, and physique — you're welcome to join! For those interested, I also organize cycling and mountain trips, and sometimes offer squash or climbing wall sessions instead of traditional gym workouts.`
    },
    exercises: {
      title: "Exercises",
      stretching: "Stretching",
      strength: "Strength",
      core: "Core",
      cardio: "Endurance"
    },
    back: "Back",
    categoryDescriptions: {
      stretching: `Stretching is an integral part of every training session and is extremely important to me. 
It helps release muscle tension, promotes relaxation, and improves joint mobility. 
Regular stretching increases body awareness and supports proper posture. 
I pay special attention to people who work long hours at a desk, for whom stretching is an essential part of training, supporting both health and daily comfort.`,

      strength: `Strength training in my sessions targets the whole body — I do not favor any single muscle group, 
because imbalances can lead to poor posture and increase the risk of injuries. 
Regular strength exercises strengthen muscles, bones, and joints, support metabolism, and help maintain healthy body weight. 
Well-developed, balanced muscles enhance stability and confidence in daily activities. 
I pay special attention to leg muscles, because strong legs support the whole body, improve balance and stability, and make everyday movements like walking, running, or climbing stairs easier.`,

      core: `Core training, targeting deep abdominal and back muscles, is key to full-body stability. 
A strong core, especially a strong abdomen, supports a healthy spine, improves balance and coordination, 
and increases efficiency in compound exercises such as squats, deadlifts, and push-ups. 
Core exercises help perform daily movements more safely and effectively. 
They are also recommended for children and teenagers, supporting proper posture, strengthening stabilizing muscles, and improving movement coordination. 
Regular core strengthening from an early age promotes healthy physical development and prevents future back problems.`,

      cardio: `Exercises like running, dancing, cycling, or skipping rope increase cardiovascular and lung capacity, 
support calorie burning, and improve physical fitness. 
Endurance is very important — it helps burn calories, improve circulation, and strengthen immunity. 
I am a supporter of cardio in forms that are enjoyable — dance, squash, mountain trips, or simple walks are, in my opinion, the best ways to stay active while having fun. 
For those interested, I organize trips to the mountains, cycling tours, or other enjoyable activities that can replace treadmill running.`
    },
    pricing: {
      title: "Pricing",
      one: "Personal training",
      pack: "Pack of 10 sessions",
      first: "First session",
      pair: "Training for two"
    },
    contact: {
      title: "Contact",
      phone: "Phone",
      email: "Email"
    }
  }
};

export default translations;
